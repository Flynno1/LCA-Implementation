...
class DAG(object):
    ...
    # Depth First Search
    def DFS_priv(self, node_list, graph, node):
        if not graph[node]:
            return True
        else:
            for child in graph[node]:
                if child not in node_list:
                    ...
                    if not self.DFS_priv(node_list, graph, child):
                        return self.DFS_priv(node_list, graph, child)
    ...
    def DFS(self, graph):
        res = True
        for node in graph:
            if not self.DFS_priv([node], graph, node):
                res = False
                break
        return res

    # LCA using Depth First Search (wrapper)
    def LCA(self, graph, node1, node2):
        global nodeList1
        global nodeList2
        nodeList1 = []
        nodeList2 = []
